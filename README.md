...
**Clipboard-first, immediate.** If the clipboard holds a token that is visible on screen and resolves (you just selected+copied the exact thing you want), `prefix+v` skips the overlay and opens it right away, routed by the same type rules. A stale clipboard from an hour ago never hijacks the picker: the text must actually be on screen.
